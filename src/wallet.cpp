...
bool CWallet::MintToTxIn(CZerocoinMint zerocoinSelected, int nSecurityLevel, const uint256& hashTxOut, CTxIn& newTxIn, CZerocoinSpendReceipt& receipt)
{
    ...
    receipt.SetStatus("Transaction Mint Started", ZLNI_TXMINT_GENERAL);
    ...
    if (!pubCoinSelected.validate()) {
        receipt.SetStatus("the selected mint coin is an invalid coin", ZLNI_INVALID_COIN);
        return false;
    }
    ...
    if (!GenerateAccumulatorWitness(zerocoinSelected, accumulator, witness, nSecurityLevel, nMintsAdded, strFailReason)) {
        receipt.SetStatus("Try to spend with a higher security level to include more coins", ZLNI_FAILED_ACCUMULATOR_INITIALIZATION);
    ...
    try {
        ...
        if (!spend.Verify(accumulator)) {
            receipt.SetStatus("the new spend coin transaction did not verify", ZLNI_INVALID_WITNESS);
            return false;
        }
        ...
        //Add the coin spend into a Loonie transaction
        ...
        try {
            ...
            receipt.SetStatus("failed to deserialize", ZLNI_BAD_SERIALIZATION);
            return false;
        }
        ...
        if (!newSpendChecking.Verify(accumulator)) {
            receipt.SetStatus("the transaction did not verify", ZLNI_BAD_SERIALIZATION);
            return false;
        }
        ...
        for (const CBigNum& item : listCoinSpendSerial) {
            if (spend.getCoinSerialNumber() == item) {
                ...
                receipt.SetStatus("the coin spend has been used", ZLNI_SPENT_USED_ZLNI);
                return false;
            }
        }
        ...
        receipt.SetStatus("CoinSpend: Accumulator witness does not verify", ZLNI_INVALID_WITNESS);
        return false;
    }

    receipt.SetStatus("Spend Valid", ZLNI_SPEND_OKAY); // Everything okay
...
bool CWallet::CreateZerocoinSpendTransaction(CAmount nValue, int nSecurityLevel, CWalletTx& wtxNew, CReserveKey& reserveKey, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vSelectedMints, vector<CZerocoinMint>& vNewMints, bool fMintChange,  bool fMinimizeChange, CBitcoinAddress* address)
{
    ...
    receipt.SetStatus("Transaction Created", ZLNI_SPEND_OKAY); // Everything okay
